...
public:
	...
	uint32_t get_index() const {
		return m_rot | (m_flip << 2) | (m_inv << 3) | (m_contrast << 4) | (m_shift << 6) | (m_median << 7) | (m_div << 8)
				| (m_rand << 9) | (m_dilate << 10) | (m_shift_x << 11) | (m_shift_y << 12) | (m_erode << 13)
				| (m_high_pass << 14);
	}
...
struct etc1_selector_palette_entry {
	...
	etc1_selector_palette_entry apply_modifier(modifier_types mod_type,
																						 const etc1_global_palette_entry_modifier &modifier) const {
		switch (mod_type) {
			case cModifierContrast: return get_contrast(modifier.m_contrast);
			case cModifierRand: return get_randomized();
			case cModifierMedian: return get_median();
			case cModifierDiv: return get_divided();
			case cModifierShift: return get_shifted(1);
			case cModifierInv: return get_inverted();
			case cModifierFlippedAndRotated: return get_flipped_and_rotated(modifier.m_flip != 0, modifier.m_rot);
			case cModifierDilate: return get_dilated();
			case cModifierShiftX: return get_shift_x();
			case cModifierShiftY: return get_shift_y();
			case cModifierErode: return get_eroded();
			case cModifierHighPass: return get_high_pass();
			default: assert(0);
				break;
		}
	...
	etc1_selector_palette_entry get_modified(const etc1_global_palette_entry_modifier &modifier,
																					 uint32_t num_order,
																					 const modifier_types *pOrder) const {
		etc1_selector_palette_entry r(*this);
...
typedef basisu::FakeStdVector<etc1_selector_palette_entry> etc1_selector_palette_entry_vec;
...
struct etc1_global_selector_codebook_entry_id {
	...
	etc1_global_selector_codebook_entry_id(uint32_t palette_index, const etc1_global_palette_entry_modifier &modifier)
			: m_palette_index(palette_index), m_modifier(modifier) {}
...
typedef basisu::FakeStdVector<etc1_global_selector_codebook_entry_id> etc1_global_selector_codebook_entry_id_vec;
...
public:
	...
	etc1_selector_palette_entry get_entry(uint32_t palette_index,
																				const etc1_global_palette_entry_modifier &modifier) const {
