...
enum transcoder_texture_format {
	...
	// Uncompressed (raw pixel) formats
			cTFRGBA32 =
			cTFTotalBlockTextureFormats, // 32bpp RGBA image stored in raster (not block) order in memory, R is first byte, A is last byte.
...
struct basisu_transcoder_state {
	...
	basisu::FakeStdVector<block_preds> m_block_endpoint_preds[2];

	enum { cMaxPrevFrameLevels = 16 };
	basisu::FakeStdVector<uint32_t> m_prev_frame_indices[2][cMaxPrevFrameLevels]; // [alpha_flag][level_index]
};
...
public:
	...
	bool transcode_slice(void *pDst_blocks,
											 ...
											 bool astc_transcode_alpha = false,
											 void *pAlpha_blocks = nullptr,
											 uint32_t output_rows_in_pixels = 0);

private:
	typedef basisu::FakeStdVector<endpoint> endpoint_vec;
	endpoint_vec m_endpoints;

	typedef basisu::FakeStdVector<selector> selector_vec;
	selector_vec m_selectors;

	const etc1_global_selector_codebook *m_pGlobal_sel_codebook;

	huffman_decoding_table m_endpoint_pred_model, m_delta_endpoint_model, m_selector_model,
			m_selector_history_buf_rle_model;
...
typedef basisu::FakeStdVector<basisu_slice_info> basisu_slice_info_vec;
...
struct basisu_file_info {
	...
	basisu::FakeStdVector<uint32_t> m_image_mipmap_levels; // the # of mipmap levels for each image
...
public:
	...
	bool get_image_level_desc(const void *pData,
														uint32_t data_size,
	...
	bool get_image_level_info(const void *pData,
														uint32_t data_size,
	...
	bool transcode_image_level(
			...
			uint32_t decode_flags = cDecodeFlagsPVRTCWrapAddressing,
			uint32_t output_row_pitch_in_blocks_or_pixels = 0,
			basisu_transcoder_state *pState = nullptr,
			uint32_t output_rows_in_pixels = 0) const;

	// Finds the basis slice corresponding to the specified image/level/alpha params, or -1 if the slice can't be found.
	int find_slice(const void *pData,
								 uint32_t data_size,
								 uint32_t image_index,
								 uint32_t level_index,
	...
	// Notes:
	// - basisu_transcoder_init() must have been called first to initialize the transcoder lookup tables before calling this function.
	bool transcode_slice(const void *pData,
											 uint32_t data_size,
											 uint32_t slice_index,
											 void *pOutput_blocks,
											 uint32_t output_blocks_buf_size_in_blocks_or_pixels,
											 block_format fmt,
											 uint32_t output_block_stride_in_bytes,
											 uint32_t decode_flags = cDecodeFlagsPVRTCWrapAddressing,
											 uint32_t output_row_pitch_in_blocks_or_pixels = 0,
											 basisu_transcoder_state *pState = nullptr,
											 void *pAlpha_blocks = nullptr,
