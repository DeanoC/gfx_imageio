...
// Types/utilities
template<typename T>
struct FakeStdVector {

	FakeStdVector(uint64_t s = 0) : vectorCapacity(0), vectorSize(0), data(nullptr) {
		resize(s);
	}
	...
	void resize(uint64_t s) {
		if (s > vectorCapacity) {
			data = (T *) MEMORY_REALLOC(data, s * sizeof(T));
			memset(data + vectorCapacity, 0, (s - vectorCapacity) * sizeof(T));
			vectorSize = s;
			vectorCapacity = s;
		} else {
			vectorSize = s;
...
typedef FakeStdVector<uint8_t> uint8_vec;
typedef FakeStdVector<int16_t> int16_vec;
typedef FakeStdVector<uint16_t> uint16_vec;
typedef FakeStdVector<uint32_t> uint_vec;
typedef FakeStdVector<uint64_t> uint64_vec;
typedef FakeStdVector<int> int_vec;
typedef FakeStdVector<bool> bool_vec;
...
template<typename T> inline void clear_vector(T &vec) { vec.fast_clear(); } //vec.erase(vec.begin(), vec.end()); }
...
enum {
	...
	cHuffmanFastLookupSize = 1 << cHuffmanFastLookupBits,
...
static const uint8_t g_huffman_sorted_codelength_codes[] =
		{cHuffmanSmallZeroRunCode, cHuffmanBigZeroRunCode, cHuffmanSmallRepeatCode, cHuffmanBigRepeatCode, 0, 8, 7, 9, 6,
		 0xA, 5, 0xB, 4, 0xC, 3, 0xD, 2, 0xE, 1, 0xF, 0x10};
const uint32_t cHuffmanTotalSortedCodelengthCodes =
		sizeof(g_huffman_sorted_codelength_codes) / sizeof(g_huffman_sorted_codelength_codes[0]);
...
inline uint32_t get_bytes_per_block(texture_format fmt) {
	switch (fmt) {
		...
		case cPVRTC1_4_RGB:
		case cPVRTC1_4_RGBA:
		case cATC_RGB: return 8;
		case cRGBA32: return sizeof(uint32_t) * 16;
		default: break;
	}
	return 16;
}
